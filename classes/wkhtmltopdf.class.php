...
class wkhtmltopdf
{
    ...
    public function setBinPath($path)
    {
        if (realpath($path) === false)
            throw new Exception('setBinPath: Path must be absolute ("'.htmlspecialchars($path,ENT_QUOTES).'")');
    ...
    public function getBin()
    {
        ...
        if (realpath($bin) === false)
            throw new Exception('getBin: Path must be absolute ("'.htmlspecialchars($bin,ENT_QUOTES).'")');
    ...
    {
        if (realpath($path) === false)
            throw new Exception('setTmpPath: Path must be absolute ("'.htmlspecialchars($path,ENT_QUOTES).'")');
    ...
    {
        if (realpath($path) === false)
            throw new Exception('setHtmlPath: Path must be absolute ("'.htmlspecialchars($path,ENT_QUOTES).'")');
    ...
    public function getHtmlPathFile()
    {
        ...
        if (realpath($file) === false)
            throw new Exception('getHtmlPathFile: Path must be absolute ("'.htmlspecialchars($file,ENT_QUOTES).'")');
    ...
    protected function _makeFilename()
    {
        ...
        if (realpath($path) === false)
            throw new Exception('_makeFilename: Path must be absolute ("'.htmlspecialchars($path,ENT_QUOTES).'")');
    ...
    protected function _render()
    {
        ...
        //echo (is_array($command)?print_r($command,true):$command);     // for debug
...
die;
    ...
    /**
                ...
                private static function _pipeExec($cmd, $input=''){
                                ...
                                $pipes = array();
        $proc = proc_open($cmd, array(array('pipe', 'r'),array('pipe', 'w'),array('pipe', 'w')), $pipes, null, null, array('binary_pipes'=>true));
                                fwrite($pipes[0], $input);
                                fclose($pipes[0]);
