...
$(function() {
				...
				$('#photoup').uploadify({
					...
					'onComplete'  : function(event, ID, fileObj, response, data) {
							...
							$('#addPhotoModal img').attr('src', response);
							...
							$('.modal-footer a').on('click',function(e){
								...
								data.img = response;
								$.post('/ajaxfunctions.php?action=crop_image', data, function(data){
									$('#addPhotoModal').modal('hide');
									$(".curPhoto img").attr("src", response+"?"+e.timeStamp);
									$(".curPhoto").show();
								},'json');
								$(this).off('click');
								 jcrop_api.destroy();
...
if(orgs){
				...
				$('#city_add .cancel').click(function(e){
				 ...
				 $('#city_add .add').click(function(e){
						...
						$.post('/ajaxfunctions.php?action=add_city',{city:$(inp).val(),country:$('select[name=country]').val()},function(data){
				...
				$('#org_add .cancel').click(function(e){
				 ...
				 $('#org_add .add').click(function(e){
						...
						var inp = $('input[name=org_add]');
						 ...
						 $.post('/ajaxfunctions.php?action=add_org',{org:$(inp).val(),city:$('select[name=city]').val()},function(data){
...
<div id="header">
				...
				<input type="hidden" name="photo" id="photo" value="<?php echo $row['photo']; ?>" autocomplete="off"/>
										<div class="control-group">
												...
												<div class="controls">
													 <input type="text" name="name" value="<?php echo htmlentities($row['name'], ENT_QUOTES, 'UTF-8'); ?>" class="span5"/>
												</div>
										</div>
										<div class="control-group">
												<label class="control-label">Surname</label>
												<div class="controls">
													<input type="text" name="surname" value="<?php echo htmlentities($row['surname'], ENT_QUOTES, 'UTF-8'); ?>" class="span5"/>
												</div>
										</div>
	...
	<?php
		...
		if($resp->num_rows){
				...
				while($rowp = $resp->fetch_assoc()){
						if(in_array($rowp['id'],$row['permisions'])){$act = 'checked="checked" ';}else{$act='';}
								echo '
...
?>
										...
										<div class="form-actions">
												<button type="submit" class="btn btn-primary" name="save" value="Save"><i class="icon-ok icon-white"></i> Save changes</button>
